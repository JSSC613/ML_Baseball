...
# --- 2. 資料前處理與特徵工程 ---
if not df.empty:
    # ==== 修正重點：無論如何都要先確保 'date' 欄位存在 ====
    if 'date' not in df.columns:
        if 'home_date' in df.columns:
            df['date'] = pd.to_datetime(df['home_date'])
        else:
            # 萬一真的找不到日期，建立假索引避免崩潰
            print("警告：找不到日期欄位，使用預設索引。")
    ...
    # 確保投手數據存在 (如果之前的 data_processing 沒跑好)
    if 'home_sp_era' not in df.columns: df['home_sp_era'] = 4.50
    if 'vis_sp_era' not in df.columns: df['vis_sp_era'] = 4.50
...
if not df.empty:
    ...
    for team in teams_list:
        ...
        if not team_games.empty:
            ...
            last_stats[team] = [0.5, 4.0, 4.0, 8.0, 0.5, 4.50, 1.35]

# --- 5. 預測單場比賽函數 ---
def predict_single_match(home, vis, model_type='xgb'):
    h_s = last_stats.get(home, [0.5, 4, 4, 8, 0.5, 4.5, 1.35])
    v_s = last_stats.get(vis, [0.5, 4, 4, 8, 0.5, 4.5, 1.35])
    ...
    diff_era = h_s[5] - v_s[5]
    diff_whip = h_s[6] - v_s[6]
    
    # 建立 DataFrame 並指定欄位名稱，解決 Feature Name 警告
    data_dict = {
...
def simulate_season():
    ...
    for league in ['AL', 'NL']:
        ...
        logs.append(f"聯盟冠軍: {cs} def {ds1 if cs!=ds1 else ds2} ({w5}-{l5})")
        ws_teams.append(cs)
